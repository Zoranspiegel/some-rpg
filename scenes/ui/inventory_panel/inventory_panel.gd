...
func handle_left_button(slot_index: int) -> void:
	if selected_slot_index >= 0 and selected_slot_index != slot_index:
		var from_item: ItemInventoryData = Inventory.get_slot_item(selected_slot_index)
		var to_item: ItemInventoryData = Inventory.get_slot_item(slot_index)
...
func _on_slot_hover(slot_index: int) -> void:
	#print("SLOT_%d_HOVER" % [slot_index])
	pass
